...
export const CalendarDayCell: React.FC<CalendarDayCellProps> = ({
  ...
  return <div onClick={() => onClick?.(date)} className={clsx("relative w-full h-full flex flex-col items-center justify-center text-sm rounded-md transition-all", isSelected && "bg-red-600 text-white", !isSelected && isToday && "border-2 border-red-600 text-red-600", isHoliday || isWeekend ? "text-red-500" : "text-white", "hover:bg-red-800/40 cursor-pointer")}>
      ...
      {activityLabels.length > 0 && <div className="absolute bottom-0 flex gap-0.5 mt-0.5 z-10">
          {activityLabels.slice(0, 3).map((label, index) => <span key={index} className={clsx("w-1.5 h-1.5 rounded-full", roColorMap[label])} />)}
        </div>}
    </div>;
};